...
const ALL_COLUMNS: { key: keyof Candidate; label: string }[] = [
  ...
  { key: 'ADMITIDO/EXCLUIDO', label: 'Admisión' },
];
...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    ...
    let filtered = data.filter(item => {
      const normalizedName = normalize(item['APELLIDOS Y NOMBRE'] || '');
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Filters Section */}
        <section className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            <div className="md:col-span-5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-[#0099cc] focus:border-transparent transition-all outline-none text-sm"
                  value={searchTerm}
            ...
            <div className="md:col-span-3">
              ...
              <select
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-[#0099cc] outline-none text-sm cursor-pointer"
                value={sedeFilter}
            ...
            <div className="md:col-span-1 flex justify-end">
              <div className="relative">
                <button
                  ...
                  className={`p-2 rounded-lg border transition-colors ${showColumnPicker ? 'bg-[#0099cc] text-white border-[#0099cc]' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                ...
                <AnimatePresence>
                  {showColumnPicker && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 p-4"
                    >
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse table-fixed">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  <AnimatePresence mode="popLayout">
                    {filteredData.slice(0, visibleCount).map((candidate, idx) => (
                      <motion.tr 
                        key={`${candidate['APELLIDOS Y NOMBRE']}-${idx}`}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
              ...
              {visibleCount < filteredData.length && (
                <div className="p-4 flex justify-center border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
                  <button
      ...
      <footer className="py-8 border-t border-slate-200 dark:border-slate-800 mt-8">
